...
├── data/                     # Data directory
├── docs/                     # Additional training notes
...
## Incremental Unified Variants

This open-source version includes two incremental variants:

- `FastWAMUnifiedShared`: shared WAN/video DiT plus one shared ActionDiT trained
  with both `wo-video` and `w-video` action losses.
- `FastWAMUnifiedTwoAction`: shared WAN/video DiT plus two ActionDiTs, one for
  `wo-video` and one for `w-video`.

The relevant task configs are:

```text
libero_unified_shared_2cam224_1e-4
libero_unified_two_action_2cam224_1e-4
robotwin_unified_shared_3cam_384_1e-4
robotwin_unified_two_action_3cam_384_1e-4
```

See [docs/INCREMENTAL_TRAINING.md](./docs/INCREMENTAL_TRAINING.md) for the
required external files, smoke commands, and static `inference_mode=wo|w`
evaluation behavior.
