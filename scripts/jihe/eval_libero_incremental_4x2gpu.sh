...
# Run the four LIBERO incremental eval jobs concurrently on one 8xH100 node.
# Jobs:
#   0,1: unified_shared without video (wo)
#   2,3: unified_shared with video (w)
...
run_eval_job() {
  ...
  while (( attempt <= MAX_EVAL_RESTARTS )); do
    echo "[${label}] attempt=${attempt}/${MAX_EVAL_RESTARTS} task=${task} mode=${inference_mode} gpus=${gpu_pair}" | tee -a "${log_file}"
    rm -rf "${out_dir}"
    mkdir -p "${out_dir}"
...
run_eval_job "unified_shared_wo" "libero_unified_shared_2cam224_1e-4" "wo" "0,1" "$@" &
pids=("$!")
run_eval_job "unified_shared_w" "libero_unified_shared_2cam224_1e-4" "w" "2,3" "$@" &
pids+=("$!")
run_eval_job "unified_two_action_wo" "libero_unified_two_action_2cam224_1e-4" "wo" "4,5" "$@" &
pids+=("$!")
run_eval_job "unified_two_action_w" "libero_unified_two_action_2cam224_1e-4" "w" "6,7" "$@" &
pids+=("$!")
...
write_combined_summary
echo "[done] all four eval jobs completed. Results: ${EVAL_OUTPUT_ROOT}"
