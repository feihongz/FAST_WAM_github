...
# Formal, resumable LIBERO Stage 2 E0/E10 label generation on one 4xH100
# JiHe node.  Four independent torchrun ranks share one immutable label job;
# the Python entrypoint deterministically assigns 16 of the 64 shards to each
...
[[ "${REQUESTED_NPROC}" == "4" ]] ||
  fail "formal LIBERO Stage 2 labels require exactly 4 H100s, got NPROC_PER_NODE=${REQUESTED_NPROC}"
readonly NPROC_PER_NODE="4"
readonly NNODES="1"

VISIBLE_GPUS="${FASTWAM_CUDA_VISIBLE_DEVICES:-0,1,2,3}"
IFS=',' read -r -a VISIBLE_GPU_TOKENS <<< "${VISIBLE_GPUS}"
[[ "${#VISIBLE_GPU_TOKENS[@]}" == "4" ]] ||
  fail "FASTWAM_CUDA_VISIBLE_DEVICES must contain exactly four comma-separated devices"
...
[stage2-labels]
  ...
  topology=1x4
  world_size=4
  rank_shards=rank_r_handles_r_r+4_..._r+60
...
if [[ "${FASTWAM_DRY_RUN}" == "1" ]]; then
  echo "[dry-run] formal 4-H100 label generation is fully planned; no files were written"
  exit 0
fi
...
if len(names) != 4 or any("H100" not in name.upper() for name in names):
    raise SystemExit(f"expected exactly four visible H100 GPUs, found {names}")
PY
